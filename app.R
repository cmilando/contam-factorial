...
ui <- fluidPage(
  ...
  fileInput("prj", "Choose PRJ to convert to JSON",
            accept = ".prj", width = "100%"
  ),
  textInput("out_f", "Base file name for outputs",
            width = "100%",
            "D:\\BU_backup\\contam_test\\ASTHMA_database\\1box\\1box"
  ),
  ...
  tabsetPanel(type = 'tabs',
    tabPanel(
      ...
      checkboxGroupInput("filter_choices",
        ...
        "Make replicants with the following filters:",
...
server <- function(input, output, session) {
  ...
  observeEvent(input$create_prj, {
    ...
    raw_json <- read_json("objs/filters.JSON", simplifyVector = T)
    reps_to_make <- subset(raw_json, name %in% input$filter_choices)

    w$show()
    # ---------------------
    # for each rep
    for (i in 1:nrow(reps_to_make)) {

      # start fresh each time
      base_json <- read_json(path = json_f, simplifyVector = T)

      # again, this will change when you get to more combos
      rep_name <- reps_to_make[i, ]$name
      # remove "_" from name to reserve for spacing character
      ...
      for (section_i in 1:length(base_json)) {
        if (section_i != 8) {
          base_json[[section_i]] <- c(base_json[[section_i]], "-999")
        } else {
          base_nr <- base_json[[8]]
          base_json[[8]] <- write_filters(reps_to_make[i, ])
        }
      }
      ...
      prj_f <- paste0(input$out_f, "_", rep_name, ".prj")
